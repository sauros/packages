...
sauros::cell_ptr _pkg_net_htons_(sauros::cells_t &cells,
                                 std::shared_ptr<sauros::environment_c> env) {
  uint16_t target = c_api_process_cell(cells[1], env)->data.i;
  return std::make_shared<sauros::cell_c>(sauros::cell_type_e::INTEGER,
...
sauros::cell_ptr _pkg_net_socket_(sauros::cells_t &cells,
                                  std::shared_ptr<sauros::environment_c> env) {
  int domain = c_api_process_cell(cells[1], env)->data.i;
  int type = c_api_process_cell(cells[2], env)->data.i;
  int protocol = c_api_process_cell(cells[3], env)->data.i;
  return std::make_shared<sauros::cell_c>(
      sauros::cell_type_e::INTEGER,
...
sauros::cell_ptr _pkg_net_close_(sauros::cells_t &cells,
                                 std::shared_ptr<sauros::environment_c> env) {
  int fd = c_api_process_cell(cells[1], env)->data.i;
...
sauros::cell_ptr _pkg_net_sockopt_(sauros::cells_t &cells,
                                   std::shared_ptr<sauros::environment_c> env) {
  int fd = c_api_process_cell(cells[1], env)->data.i;
  int level = c_api_process_cell(cells[2], env)->data.i;
  int opt_name = c_api_process_cell(cells[3], env)->data.i;
  int opt = 1;
  return std::make_shared<sauros::cell_c>(
      sauros::cell_type_e::INTEGER,
...
sauros::cell_ptr _pkg_net_bind_(sauros::cells_t &cells,
  ...
  address->sin_family = c_api_process_cell(cells[2], env)->data.i;
  address->sin_port = c_api_process_cell(cells[3], env)->data.i;
  address->sin_addr.s_addr = c_api_process_cell(cells[4], env)->data.i;
...
sauros::cell_ptr _pkg_net_listen_(sauros::cells_t &cells,
                                  std::shared_ptr<sauros::environment_c> env) {
  int fd = c_api_process_cell(cells[1], env)->data.i;
  int n = c_api_process_cell(cells[2], env)->data.i;
...
sauros::cell_ptr _pkg_net_read_(sauros::cells_t &cells,
                                std::shared_ptr<sauros::environment_c> env) {
  int fd = c_api_process_cell(cells[1], env)->data.i;
  int buffer_len = c_api_process_cell(cells[2], env)->data.i;
...
sauros::cell_ptr _pkg_net_send_(sauros::cells_t &cells,
  ...
  int flags = c_api_process_cell(cells[3], env)->data.i;
...
_pkg_net_shutdown_(sauros::cells_t &cells,
                   std::shared_ptr<sauros::environment_c> env) {
  int fd = c_api_process_cell(cells[1], env)->data.i;
  int how = c_api_process_cell(cells[2], env)->data.i;
