...
#define SELF_CHECK                                                             \
  if (!fc) {                                                                   \
    throw exceptions::runtime_c("file_io > Target file cell no longer exists", \
                                cells[0]);                                     \
  }
...
extern cell_ptr _pkg_file_io_get_handle_(cells_t &cells, env_ptr env) {
  ...
  box->inner_env->set(
      "open",
      std::make_shared<cell_c>([fc](cells_t &cells, env_ptr env) -> cell_ptr {
  ...
  box->inner_env->set(
      ...
      std::make_shared<cell_c>([fc](cells_t &cells, env_ptr env) -> cell_ptr {
        ...
        return std::make_shared<cell_c>(cell_type_e::INTEGER, length,
                                        cells[0]->location);
      }));
  ...
  box->inner_env->set(
      ...
      std::make_shared<cell_c>([fc](cells_t &cells, env_ptr env) -> cell_ptr {
        ...
        auto result = std::make_shared<cell_c>(
            cell_type_e::STRING, result_string, cells[0]->location);
  ...
  box->inner_env->set(
      ...
      std::make_shared<cell_c>([fc](cells_t &cells, env_ptr env) -> cell_ptr {
        ...
        auto result = std::make_shared<cell_c>(cell_type_e::STRING, data,
                                               cells[0]->location);
